...
// Five visual templates; pick one via the `template` option. Pure canvas (no
// fonts to load) so it works offline and never blocks gameplay.
 ...
 * @param {{name, score, accuracy, split, text, template}} opts
 */
export function generateShareCard({ name, score, accuracy, split, text, template = 'neon' }) {
  const W = 1080;
  const H = 1920;
...
const TEMPLATES = {
  ...
  neon(ctx, d) {
    ...
    ring(ctx, CX, 900, 360, 'rgba(0,229,192,0.16)');
    ring(ctx, CX, 900, 300, 'rgba(255,255,255,0.04)');

    txt(ctx, d.name, CX, 660, `900 78px ${SANS}`, '#ffffff');
    txt(ctx, spaced(text.cardLabelScore), CX, 800, `700 32px ${SANS}`, slate(0.92));
    glowText(ctx, d.score, CX, 978, `900 230px ${SANS}`, ACCENT, 'rgba(0,229,192,0.4)', 48);

    statPair(ctx, CX, 1280, d, { value: '#fff', label: slate(0.92), divider: 'rgba(255,255,255,0.10)' });
    txt(ctx, text.cardTagline, CX, 1720, `600 40px ${SANS}`, 'rgba(226,232,240,0.9)');
    txt(ctx, 'aimku.xyz', CX, 1800, `800 50px ${SANS}`, ACCENT);
  },

  // 2) MINIMAL — light background, ink type, one hairline accent. Clean.
  minimal(ctx, d) {
    const { W, H, CX, text } = d;
    const INK = '#0f1419';
    const SUB = '#5b6672';
    const A = '#0bbfa6'; // darker cyan for contrast on light
    ...
    txt(ctx, d.name, CX, 720, `900 76px ${SANS}`, INK);
    txt(ctx, spaced(text.cardLabelScore), CX, 850, `700 30px ${SANS}`, SUB);
    txt(ctx, d.score, CX, 1060, `900 220px ${SANS}`, INK);
    bar(ctx, CX, 1110, 160, 6, A);

    statPair(ctx, CX, 1380, d, { value: INK, label: SUB, divider: 'rgba(15,20,25,0.12)' });
    txt(ctx, text.cardTagline, CX, 1720, `600 38px ${SANS}`, SUB);
    txt(ctx, 'aimku.xyz', CX, 1800, `800 48px ${SANS}`, A);
  },

  // 3) AGENT — red-dominant, bold diagonal stripe, aggressive.
  agent(ctx, d) {
    const { W, H, CX, text } = d;
    ...
    txt(ctx, d.name, CX, 700, `900 80px ${SANS}`, '#ffffff');
    txt(ctx, spaced(text.cardLabelScore), CX, 840, `700 32px ${SANS}`, 'rgba(255,180,186,0.95)');
    glowText(ctx, d.score, CX, 1020, `900 230px ${SANS}`, '#ffffff', 'rgba(255,70,85,0.55)', 50);

    statPair(ctx, CX, 1320, d, { value: '#fff', label: 'rgba(255,180,186,0.95)', divider: 'rgba(255,70,85,0.3)' });
  ...
  gradient(ctx, d) {
    ...
    txt(ctx, d.name, CX, 720, `900 76px ${SANS}`, '#ffffff');
    txt(ctx, spaced(text.cardLabelScore), CX, 850, `700 30px ${SANS}`, 'rgba(255,255,255,0.8)');
    glowText(ctx, d.score, CX, 1040, `900 220px ${SANS}`, '#ffffff', 'rgba(255,255,255,0.45)', 40);

    statPair(ctx, CX, 1340, d, { value: '#fff', label: 'rgba(255,255,255,0.85)', divider: 'rgba(255,255,255,0.3)' });
  ...
  mono(ctx, d) {
    ...
    txt(ctx, '// VALORANT_AIM_TRAINER', CX, wy + 180, `700 30px ${MONO}`, ACCENT);
    txt(ctx, `user: ${d.name}`, CX, wy + 300, `700 52px ${MONO}`, '#e2e8f0');
    txt(ctx, `> ${text.cardLabelScore.toLowerCase().replace(/\s/g, '_')}`, CX, wy + 420, `600 30px ${MONO}`, 'rgba(148,163,184,0.85)');
    glowText(ctx, d.score, CX, wy + 600, `800 210px ${MONO}`, ACCENT, 'rgba(0,229,192,0.4)', 44);

    txt(ctx, `acc=${d.acc}   split=${d.split}`, CX, wy + 760, `600 40px ${MONO}`, '#e2e8f0');
    bar(ctx, CX, wy + 820, ww - 120, 2, 'rgba(0,229,192,0.25)');
    txt(ctx, text.cardTagline, CX, wy + 940, `600 34px ${MONO}`, 'rgba(148,163,184,0.9)');
...
// Accuracy | split pair split by a centre hairline. `c` = colours.
