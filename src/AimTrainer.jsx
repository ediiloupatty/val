...
export default function AimTrainer({ onExit, lang, setLang, isMobile, name, setName, best, setBest, onSession, onRoundStart, showToast }) {
  ...
  const trackingAccRef = useRef(0);
  const trackingAvgSwitchRef = useRef(0);
  const trackingComboRef = useRef(1.0);
  ...
  useEffect(() => {
    ...
    engine.current.onTrackingScore = (pts) => {
      ...
      if (el) el.innerText = scoreRef.current;
      
      if (!pbBeatenRef.current) {
        const target = pbTargetRef.current;
        if (target > 0 && scoreRef.current > target) {
          pbBeatenRef.current = true;
          // React update is OK here since it's a one-time event per round.
          setPbBeaten(true);
        }
      }
    };
    ...
    engine.current.onTrackingStats = (acc, avgSwitchMs, combo) => {
      trackingAccRef.current = acc;
      trackingAvgSwitchRef.current = avgSwitchMs;
      trackingComboRef.current = combo;
      const elCombo = document.getElementById('live-tracking-combo');
      if (elCombo) elCombo.innerText = combo > 1 ? `${combo.toFixed(1)}x` : `${acc.toFixed(0)}%`;
    };
  ...
  const endGame = useCallback(() => {
    ...
    setCountdown(null);
    setScore(scoreRef.current);
    setTrackingAccuracy(trackingAccRef.current);
    setTrackingAvgSwitch(trackingAvgSwitchRef.current);
    setTrackingComboDisplay(trackingComboRef.current);
    engine.current?.stopTrackingTone?.();
  ...
  const startPractice = useCallback(() => {
    ...
    scoreRef.current = 0;
    trackingAccRef.current = 0;
    trackingAvgSwitchRef.current = 0;
    trackingComboRef.current = 1.0;
      ...
      <main className="absolute inset-0">
        ...
        {isRunning && (
          <div className="pointer-events-none absolute left-1/2 top-5 flex -translate-x-1/2 gap-8 rounded-full border border-white/10 bg-black/40 px-6 py-2.5 text-sm font-bold tabular-nums shadow-md">
            <span id="live-score" className="text-val-accent">{score}</span>
            <span id="live-tracking-combo" className="text-slate-300">
